...
public class SalaryUtils {
    ...
    public static double calculateAnnualSalary(double monthlySalary) {
        // checks if monthlySalary is negative and throws an excpeption
        if(monthlySalary <= 0) {
    ...
    public static double calculateTax(double annualSalary) {
        // checks if annualSalary is negative and throws an excpeption
        if (annualSalary <= 0 ) {
            throw new IllegalArgumentException("Annual salary must not be negative");
        }
        // checks if annual salary is less than 10000
        if(annualSalary < 10000) {
            return 0;
        // checks if annual salary falls within range 
    ...
    public static double calculateNetSalary(double annualSalary) {
        // checks if annualSalary is negative
        if(annualSalary <= 0) {
            throw new IllegalArgumentException("Annual salary must not be negative");
        }
    ...
    public static double applyRaise(double currentSalary, double percentRaise) {
        // checks if currentSalary is negative
            if(currentSalary <= 0) {
                throw new IllegalArgumentException("Current salary must be positive");
            }
            // checks if percentRaise falls within range
